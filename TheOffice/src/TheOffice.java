...
public class TheOffice {
	...
	TheOffice(String fileLoc) throws TaskLevelException{
		...
		int size = Integer.parseInt(fs.nextLine().trim());
		...
		for(int i = 0; i < size; i++){
			...
			if(type.equals("RegionalManager")){
				...
				//Grab min task for regional manager
				minimumTask = Integer.parseInt(fs.nextLine().trim());
				
				AssistantRegionalManager assistant = new AssistantRegionalManager(IDNumber, name, taskList, manager, minimumTask);
				
				// Set the assistant manager for the office if it is not already set
				if (assistantManager == null){
					assistantManager = assistant;
				}
				
				// Add employee to worker array
				workers[i] = assistant;
			}
				...
				SalesAssociate associate = new SalesAssociate(IDNumber, name, taskList, numClients, products, manager);
				...
				Receptionist receptionist = new Receptionist(IDNumber, name, taskList, assistantManager, maxTaskComplexityLevel);
	...
	// Get workers for The Office
	public Employee[] getWorkers(){
		return this.workers;
	}
	
	// Set workers for this class
	public void setWorkers(Employee[] workers){
		this.workers = workers;
	}
	
	// Makes and returns a string that has employees’ name and their minimum task level
	public String levelDisplay(){
		String returnString = "";
		Employee[] workers = this.getWorkers();
		for(int i = 0; i < workers.length; ++i){
			returnString += "Name: " + workers[i].getName() + "\t";
			if(workers[i] instanceof RegionalManager){
				RegionalManager regMan = (RegionalManager) workers[i];
			    returnString += "Level: " + regMan.getMinimumTask();
			} else if ( workers[i] instanceof AssistantRegionalManager){
				AssistantRegionalManager assistMan = (AssistantRegionalManager) workers[i];
			    returnString += "Level: " + assistMan.getMinimumTask();
			} else {
	...
	public String toString(){
		...
		Employee [] workers = this.getWorkers();
		for(int i = 0; i < workers.length; ++i){
			if(workers[i].getClass().getName().equals("RegionalManager")){
